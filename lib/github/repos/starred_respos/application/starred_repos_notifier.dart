...
class StarredReposState with _$StarredReposState {
  ...
  const factory StarredReposState.initial(Fresh<List<GithubRepo>> repos) = _Initial;
...
class StarredReposNotifier extends StateNotifier<StarredReposState> {
  ...
  StarredReposNotifier(this._repository) : super(StarredReposState.initial(Fresh.yes([])));

  int _page = 1;

  Future<void> getNextStarredReposPage() async {
    state = StarredReposState.loadInProgress(state.repos, PaginationConfig.itemsPerPage);
